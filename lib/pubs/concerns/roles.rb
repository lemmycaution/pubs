  ...
  module Concerns
    module Roles
      ...
      def can? action, klass
        ...
        !cannot?(action,klass) and (can_manage_all? or can_manage?(klass) or ability == ALL or ability.try(:include?,klass.to_s))
      end

      def cannot? action, klass
        able?(:cannot,action).try(:include?,klass.to_s)
      end
      ...
      def can_manage? klass
        able?(:can,:manage).try(:include?, klass.to_s)
      end
